...
<body class="bg-brand-50 text-brand-900 h-screen overflow-hidden" style="--brand-accent: {{ auth()->user()?->branch?->accent_color ?? '#2563eb' }}" x-data="connectionStatus">
    <div class="h-full flex flex-col">
        ...
        <header class="bg-brand-100 border-b border-brand-200 px-6 py-3 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="font-semibold text-brand-900 leading-tight">BranchOps POS</h1>
                        <p class="text-xs text-brand-600">{{ auth()->user()->branch->name ?? 'Main Branch' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <!-- User Menu -->
                <div class="flex items-center gap-3 pl-4 border-l border-brand-200">
                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-medium text-brand-900">{{ auth()->user()->name }}</p>
                        <p class="text-xs text-brand-600">{{ auth()->user()->role }}</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="inline">
                        @csrf
                        <button type="submit"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-brand-600">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" stroke-linejoin="miter" aria-hidden="true">
        ...
        <!-- Main Content Area -->
        <main class="flex-1 overflow-hidden bg-brand-50">
            @yield('content')
        </main>

        <footer class="shrink-0 border-t border-brand-200 bg-brand-100 px-6 py-2 flex items-center justify-between text-xs text-brand-600">
        ...
        <div id="connection-status" class="fixed bottom-12 right-4 px-3 py-1.5 text-xs font-medium border"
             :class="connectionStatus === 'Connected' ? 'bg-brand-100 text-success border-success' : 'bg-brand-100 text-danger border-danger'">
            <span class="inline-block w-2 h-2 mr-2" :class="connectionStatus === 'Connected' ? 'bg-success' : 'bg-danger'"></span>
